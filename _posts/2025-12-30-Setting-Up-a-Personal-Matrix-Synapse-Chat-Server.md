...
### You will need to sign up for/already have a few services for this project:
...
The first change we will make is to the database supporting Synapse. Initially, it ships with SQLite, but we will install PostgreSQL and edit our `homeserver.yaml` file for the change. Install and connect to PostgreSQL to set up a user and db for Synapse:
...
Will Lewis gives an example of the VirtualHost file implementation on [his blog](https://willlewis.co.uk/blog/posts/run-your-own-matrix-server-guide/), which we can add as a .conf file in our Apache2 sites-enabled directory:
...
Instead of port forwarding with our router, we will instead use a Cloudflare tunnel for a more secure connection. Install the Cloudflare tunnel via Docker and make sure it points inside the Docker container at https://host.docker.internal:443 so it can communicate with Apache locally.

Finally, we need to configure the .well-known files in either Apache or Cloudflare. These files help other clients and servers locate and discover your Matrix server through the tunnel. To reduce the load on the Apache server, we will intercept these requests with Cloudflare by using a Cloudflare worker:
...
*Note the `-p 127.0.0.1:8008:8008` instead of `-p 8008:8008`, so Synapse listens only to your local Apache server and is not exposed publicly.*
...
Fire up your Element app and log in!
